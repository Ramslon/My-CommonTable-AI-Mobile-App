...
class _StudentFeaturesScreenState extends State<StudentFeaturesScreen> {
  ...
  late List<MealSuggestion>
  _mentalHealth; // curated list: mood-boosting, stress-reducing
  ...
  Future<void> _getAiMoodAdvice() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to get AI suggestions: $e')),
      );
    } finally {
      if (mounted) {
  ...
  Future<void> _saveMoodLogToFirestore({
    required String key,
  ...
  Future<void> _generate() async {
    ...
    final withinBudget = pool
        .where((m) => m.totalCost <= targetPerMeal * 1.1)
        .toList();
    ...
    if (chosen.isEmpty && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'No meals matched your filters. Showing budget-friendly options.',
          ),
        ),
      );
    }
                ...
                child: Column(
                  ...
                  children: [
                    ElevatedButton.icon(
                      ...
                      label: Text(
                        'View Shopping List (${_selected.length} selected)',
                      ),
                    ),
  ...
  Future<void> _cacheDeals() async {
    try {
      ...
      await prefs.setString(
        'student_cached_deals',
        jsonEncode(_deals.map((e) => e.toJson()).toList()),
      );
    } catch (_) {}
  }
  ...
  Future<void> _loadCachedDeals() async {
    try {
      ...
      if (raw != null) {
        ...
        _deals = arr
            .map((e) => _LocalOffer.fromMap(Map<String, dynamic>.from(e)))
            .toList();
        if (mounted) setState(() {});
      }
    } catch (_) {}
  ...
  Future<List<_LocalOffer>> _loadDealsFromAssets() async {
    try {
      final raw = await rootBundle.loadString(
        'assets/data/promotions_mock.json',
      );
      final arr = (jsonDecode(raw) as List).cast<Map>();
      return arr
          .map((e) => _LocalOffer.fromMap(Map<String, dynamic>.from(e)))
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_deals.isEmpty)
              Text(
                'No deals yet. Pull to refresh or try again later.',
            ...
            ..._deals
                ...
                .map(
                  (d) => ListTile(
                    ...
                    trailing: d.price != null
                        ? Text(_currency.format(d.price))
                        : null,
                    onTap: () async {
  ...
  Future<void> _loadCafeteriasFromAssets() async {
    try {
      final raw = await rootBundle.loadString(
        'assets/data/cafeterias_mock.json',
      );
      final arr = (jsonDecode(raw) as List).cast<Map>();
      _cafeterias = arr
          .map((e) => CafeteriaEntry.fromMap(Map<String, dynamic>.from(e)))
          .toList();
      if (mounted) setState(() {});
    } catch (_) {
      _cafeterias = const [];
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Find healthier picks at campus dining or restaurants near you.',
              style: TextStyle(color: Colors.black.withValues(alpha: 0.7)),
            ),
            const SizedBox(height: 8),
            Row(
            ...
            ..._eateries
                ...
                .map(
                  (e) => ListTile(
                    ...
                    trailing: e.rating != null
                        ? _RatingChip(
                            rating: e.rating!,
                            count: e.userRatingsTotal,
                          )
                        : null,
                    onTap: () async {
            ...
            ..._cafeterias
                ...
                .map(
                  (c) => ListTile(
                    title: Text(c.name),
                    subtitle: Text(
                      c.healthiest?.join(', ') ?? 'Healthy picks available',
                    ),
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        for (final r in results) {
          if (r is Map)
            list.add(EateryResult.fromMap(Map<String, dynamic>.from(r)));
        }
              ...
              children: [
                ElevatedButton.icon(
                  ...
                  onPressed: () => Navigator.of(
                    context,
                  ).pushNamed(AppRoutes.socialCommunity),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Choose a mood to get supportive nutrition ideas',
              style: TextStyle(color: Colors.black.withValues(alpha: 0.7)),
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                onPressed: (_selectedMoodKey == null || _adviceLoading)
                    ? null
                    : _getAiMoodAdvice,
                ...
                label: Text(
                  _adviceLoading
                      ? 'Getting suggestions…'
                      : 'Get AI Suggestions for My Mood',
                ),
              ),
            ),
        ...
        child: Column(
          ...
          children: [
              ...
              builder: (context, constraints) {
                ...
                final regionField = DropdownButtonFormField<String>(
                  ...
                  items: const [
                    DropdownMenuItem(
                      value: 'Default',
                      child: Text('Region: Default'),
                    ),
                    DropdownMenuItem(
                      value: 'Nigeria',
                      child: Text('Region: Nigeria'),
                    ),
                    DropdownMenuItem(
                      value: 'Ghana',
                      child: Text('Region: Ghana'),
                    ),
                    DropdownMenuItem(
                      value: 'Kenya',
                      child: Text('Region: Kenya'),
                    ),
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Region presets (local staples)',
                  ),
                );
                if (isNarrow) {
                  return regionField;
            ...
            SwitchListTile(
              contentPadding: EdgeInsets.zero,
              title: const Text(
                'Use local staples (e.g., rice, beans, plantain, oats)',
              ),
  ...
  List<MealSuggestion> _buildMentalHealthSuggestions([
    ...
    final base = <MealSuggestion>[
      MealSuggestion(
        ...
        ingredients: const [
          ...
          IngredientItem(name: 'Banana (1)', cost: 80.0),
          IngredientItem(
            name: 'Peanuts or mixed nuts (small handful)',
            cost: 120.0,
          ),
        ],
      ),
      ...
      MealSuggestion(
        ...
        ingredients: const [
          ...
          IngredientItem(
            name: 'Peanuts/mixed nuts (small handful)',
            cost: 120.0,
          ),
          IngredientItem(
            name: 'Raisins/dried fruit (small handful)',
            cost: 80.0,
          ),
        ],
      ),
              ...
              children: [
                ...
                Column(
                  children: [
                    ...
                    Text(
                      _formatCost(suggestion.totalCost),
                      style: const TextStyle(fontWeight: FontWeight.w700),
                    ),
                  ],
                ),
...
class IngredientItem {
  final String name;
  final double
  cost; // approximate per recipe serving for this ingredient portion
